...
import { IGestionCompraRepository, ListarComprasResult, MensajeCompra } from '../../domain/gestion-compra.repository';
...
export class GestionCompraPrismaRepository implements IGestionCompraRepository {
    ...
    async listar(filtros?: any): Promise<ListarComprasResult> {
        try {
            ...
            if (filtros?.buscar) {
                ...
                conditions.push(Prisma.sql`(gc.descri_prod LIKE ${searchTerm} OR gc.area LIKE ${searchTerm} OR us.nombres LIKE ${searchTerm})`);
            }
            ...
            if (filtros?.estado_autorizacion !== undefined) {
                conditions.push(Prisma.sql`gc.estado_autorizacion = ${filtros.estado_autorizacion}`);
            }
    ...
    async crearMensaje(solicitudId: bigint, nitUsuario: number, mensaje: string): Promise<boolean> {
        try {
    ...
    async enviarAutorizacion(solicitudId: bigint, comentarios: string, archivos: string[]): Promise<boolean> {
        try {

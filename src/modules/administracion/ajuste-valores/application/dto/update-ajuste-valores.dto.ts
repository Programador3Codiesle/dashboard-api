...
export class UpdateAjusteValoresDto {
    @IsOptional()
    @IsNumber()
    @ApiProperty({ example: 1000, description: 'Retención en la fuente', required: false })
    retencion?: number;
    ...
    @IsOptional()
    @IsNumber()
    @ApiProperty({ example: 100, description: 'Avisos y tableros', required: false })
    Retencion_estampilla1?: number;

    @IsOptional()
    @IsNumber()
    @ApiProperty({ example: 500, description: 'Sobretasa bomberil', required: false })
    valor_aplicado?: number;

    @IsOptional()
    @IsNumber()
    @ApiProperty({ example: 5000, description: 'Valor aplicado', required: false })
    valor_total?: number;

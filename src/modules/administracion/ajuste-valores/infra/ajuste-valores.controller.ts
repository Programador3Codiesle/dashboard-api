...
export class AjusteValoresController {
    ...
    @Get('valores2')
    obtenerValores2(@Query('tipo') tipo: string, @Query('numero') numero: string) {
        return this.ajusteValoresFacade.obtenerValores2(tipo, Number(numero));
    }

    @Get('valores-cruce')
    obtenerValoresCruce(@Query('tipo') tipo: string, @Query('numero') numero: string) {
        return this.ajusteValoresFacade.obtenerValoresCruce(tipo, Number(numero));
    }

    @Get('documentos-cerrados')
    validarDocumentosCerrados(@Query('ano') ano: string, @Query('mes') mes: string) {
        return this.ajusteValoresFacade.validarDocumentosCerrados(Number(ano), Number(mes));
    }
    
    @Put(':numero')
    actualizarValores(@Param('numero') numero: string,@Query('tipo') tipo: string, @Body() dto: UpdateAjusteValoresDto) {
        return this.ajusteValoresFacade.actualizarValores(Number(numero), tipo, dto);
    }
}
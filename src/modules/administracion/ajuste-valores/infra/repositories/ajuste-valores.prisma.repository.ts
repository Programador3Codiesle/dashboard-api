...
export class AjusteValoresPrismaRepository implements IAjusteValoresRepository {
  ...
  async obtenerValores(
    ...
    try {
      ...
      const entity = new AjusteValoresEntity({
        ...
        retencion_iva:
          data.retencion_iva != null ? Number(data.retencion_iva) : null,
        retencion_ica:
          data.retencion_ica != null ? Number(data.retencion_ica) : null,
        iva: data.iva != null ? Number(data.iva) : null,
        Retencion_estampilla2:
          data.Retencion_estampilla2 != null
            ? Number(data.Retencion_estampilla2)
            : null,
        Retencion_estampilla1:
          data.Retencion_estampilla1 != null
            ? Number(data.Retencion_estampilla1)
            : null,
        valor_aplicado:
          data.valor_aplicado != null ? Number(data.valor_aplicado) : null,
  ...
  async actualizarValores(
    ...
    try {
      ...
      const result = await this.executeUpdateAndSelect(
        updateFields,
        updateConfig,
      );
  ...
  private buildUpdateFields(
    ...
    for (const [entityField, dbField] of fieldMappings) {
      ...
      if (data[entityField] !== undefined) {
        updateFields.push(
          Prisma.sql`${Prisma.raw(dbField)} = ${data[entityField]}`,
        );
      }
    }
  ...
  private getUpdateConfig(
    ...
    const contieneFormaPago =
      data.forma_pago !== undefined || data.forma_pago2 !== undefined;
    ...
    return {
      ...
      entityMapper: (row: any) => ({
        ...
        retencion_iva:
          row.retencion_iva != null ? Number(row.retencion_iva) : null,
        retencion_ica:
          row.retencion_ica != null ? Number(row.retencion_ica) : null,
        Retencion_estampilla2:
          row.Retencion_estampilla2 != null
            ? Number(row.Retencion_estampilla2)
            : null,
        Retencion_estampilla1:
          row.Retencion_estampilla1 != null
            ? Number(row.Retencion_estampilla1)
            : null,
        valor_aplicado:
          row.valor_aplicado != null ? Number(row.valor_aplicado) : null,

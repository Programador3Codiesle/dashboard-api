...
export class TecnicoService {
  ...
  async buildTecnicos(
    ...
    let npsInt = 0;
      const npsTecRows = await this.tecnicoRepo.getNpsByTecBuscar(nitUsuario, mes, ano);
    ...
    const switchSede = tecnicosSedeSwitch(sedesUsu);
      const dataRanTec = switchSede.sedesVentasRanking
          ? await this.tecnicoRepo.getVentasTecRanking(switchSede.sedesVentasRanking, mes, ano)
          : [];
      const topeRanPres = switchSede.topeRanPres;

      let ranVendidoSede = 0,
      ranNpsSede = 0;
      const sedesRankVentas = await this.tecnicoRepo.getRankingVentas(switchSede.sedesRanking);
      const sedesRankNps = await this.tecnicoRepo.getRankingNps(switchSede.sedesRanking);
    ...
    for (let m = 1; m <= mes; m++) {
      ...
      const npsIntRows = await this.tecnicoRepo.getNpsByTecBuscar(nitUsuario, m, ano);
      if (npsIntRows.length > 0) {
        ...
        const npsMes =
          totalEncuestas > 0
            ? ((enc9a10 - enc0a6) / totalEncuestas) * 100
            : 0;
      ...
      const npsGmRows = await this.tecnicoRepo.getNpsByTecGmGraf(nitUsuario, m, ano);
